...
class API {
    ...
    static async exportPagesAsImages(fileId, pages = [], dpi = 300, format = 'png', exportPath = '') {
        return await this.request('/export-pages-as-images', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                file_id: fileId,
    ...
    static async convertToHTML(fileId, mode = 'text') {
        return await this.request('/convert-to-html', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
