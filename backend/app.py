...
@app.route('/api/export-pages-as-images', methods=['POST'])
def export_pages_as_images():
    ...
    try:
        task_id = task_manager.submit_task(
            pdf_tasks.export_pages_as_images_task,
            file_id,
            pages if pages else None,
            dpi,
            image_format,
            export_path if export_path else None
        )
...
def convert_to_html():
    ...
    data = request.json
    file_id = data.get('file_id')
    mode = data.get('mode', 'text')  # text, image, hybrid
    ...
    try:
        task_id = task_manager.submit_task(
            pdf_tasks.convert_to_html_task,
            file_id,
            mode
        )
...
def download_file(file_id):
    """下载处理后的PDF或Word文件 - 跨平台兼容版本"""
    folder = request.args.get('folder', 'processed')
    filepath = get_file_path(file_id, folder)
    
    if not filepath or not os.path.exists(filepath):
        # 增强错误日志
        print(f"[下载错误] 文件不存在: file_id={file_id}, folder={folder}, filepath={filepath}")
        return jsonify({'error': '文件不存在'}), 404
    ...
    try:
        ...
        if file_ext == '.docx':
            ...
            # 清理文件名: 移除_word后缀,提供干净的下载文件名
            clean_file_id = file_id.replace('_word', '')
            download_name = f"{clean_file_id}.docx"
        else:
            mimetype = 'application/pdf'
            download_name = f"{file_id}.pdf"
        
        # 成功日志
        print(f"[下载成功] file_id={file_id}, filepath={filepath}, download_name={download_name}")
        ...
        print(f"[下载异常] file_id={file_id}, error={str(e)}")
