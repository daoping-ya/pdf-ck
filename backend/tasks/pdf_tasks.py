...
def export_pages_as_images_task(task_id, file_id, pages=None, dpi=300, image_format='png', export_path=None):
    """导出PDF页面为高质量图片的任务"""
    result = PDFService.export_pages_as_images(file_id, pages, dpi, image_format, export_path, progress_callback=_get_progress_callback(task_id))
    return result

def convert_to_html_task(task_id, file_id, mode='text'):
    """转换PDF为HTML的任务"""
    result = PDFService.convert_to_html(file_id, mode, progress_callback=_get_progress_callback(task_id))
    return result
